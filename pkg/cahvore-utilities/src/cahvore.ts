 ...
 */
function cmod_cahvore_2d_to_3d_general(
    pos2: Vector2,
    linearity: number,
    c: Vector3,
    a: Vector3,
    ...
    assert(Boolean(pos2), 'CAHVORE 2d to 3d pos2 is empty');
...
function cmod_cahv_2d_to_3d(
    ...
    /* Check input */
    assert(Boolean(pos2), 'CAHV 2d to 3d pos2 is empty');
...
function cmod_cahvor_2d_to_3d(
    ...
    const tau = lambda.dot(lambda) / omega_2;
    const k1 = 1 + r.x; /*  1 + rho0 */
    const k3 = r.y * tau; /*  rho1*tau  */
    const k5 = r.z * tau * tau; /*  rho2*tau^2  */
    let mu = r.x + k3 + k5;
